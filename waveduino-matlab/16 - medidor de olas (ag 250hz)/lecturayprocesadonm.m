...
%% High-pass filter linear giroscope to remove drift
...
% Plot
figure('Number', 'off', 'Name', 'High-pass filtered Gyroscope');
hold on;
plot(gyr(:,1), 'r');
plot(gyr(:,2), 'g');
plot(gyr(:,3), 'b');
...
% Plot
figure('Number', 'off', 'Name', 'High-pass filtered Linear Acceleration');
hold on;
plot(linAcc(:,1), 'r');
plot(linAcc(:,2), 'g');
plot(linAcc(:,3), 'b');
xlabel('sample');
ylabel('m/s²');
title('High-pass filtered linear acceleration');
legend('X', 'Y', 'Z');
