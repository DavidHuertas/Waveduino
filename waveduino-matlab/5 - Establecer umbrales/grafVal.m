...
%Inicializamos las variables necesarias para el medidor de umbral:
...
while (get(button, 'UserData'))
    ...
    %Código contador de umbral:
    
    if (IMU(1,1) > limval && arriba==0)
        cont=cont+1;
        arriba=1;
    end
    ...
    subplot(2,1,1)
    plot(i,axdatosfilt,'r', i,limval*ones(buffer,1),'b--');
