% getSetupBlockUserData(modelHandle, arduinoUnit)
%  Searches the model for the Arduino IO block specified by <arduinoUnit> (e.g., 'Arduino1')
...
%  arduinoUnit is a string, corresponding to the arduinoVar parameter (e.g., 'Arduino1')
%
% Example:
...
function data = getSetupBlockUserData(modelHandle, arduinoUnit)

blockName = find_system(modelHandle,...
    'SearchDepth', 1, ...
    'MaskType', 'Arduino IO Setup', ...
    'FunctionName', 'msfun_arduino_io_setup', ...
    'arduinoVar', arduinoUnit);

if numel(blockName) == 0
    error('Cannot find Arduino IO Setup Block for ''%s'' at top level of model', arduinoUnit);
elseif numel(blockName) > 1
    error('Multiple Arduino IO Blocks for ''%s''', arduinoUnit);
end

data = get_param(blockName, 'UserData');
if isempty(data)
   error('Arduino IO Setup Block for ''%s'' is not initialized', arduinoUnit); 
end
