...
%%
    function InitConditions(block) 
        ...
        myServoNum = str2double(block.DialogPrm(2).Data);     
        servoVarName = sprintf('servo%d', myServoNum);
        if ~customData.isKey(servoVarName)        
            % servo isn't already attached
            myArduino.servoAttach(myServoNum);
...
%%
    function Output(block)                
        % fprintf('%s: Output\n', getfullname(block.BlockHandle));
        myArduino.servoWrite(myServoNum, min(180,max(0,round(block.InputPort(1).Data))) );
    end
