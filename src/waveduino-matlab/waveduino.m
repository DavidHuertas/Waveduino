...
hold on;
plot(tiempo(1:n-nrf), posHP(:,1), 'r');
plot(tiempo(1:n-nrf), posHP(:,2), 'g');
plot(tiempo(1:n-nrf), posHP(:,3), 'b');
xlabel('Tiempo (s)');
ylabel('Posicion (m)');
...
hold on;
plot(tiempo, acel(:,2), 'r');
plot(tiempo, acel(:,1), 'g');
...
ac_umbral = mean(ac_modFilt) + 0.5*std(ac_modFilt);
set(handles.acelumb,'String',num2str(ac_umbral));
...
% Imponer el principio y el final en el umbral, evitar errores
umbral(1:3)=1;
umbral((n-3):n);

quat = zeros(length(tiempo), 4);
AHRSmedir = AHRS('SamplePeriod', dt, 'Kp', 1, 'KpInit', 1);
...
for t = 1:length(tiempo)
    ...
    AHRSmedir.UpdateIMU(deg2rad([gx(t) gy(t) gz(t)]), [ax(t) ay(t) az(t)]);
    quat(t,:) = AHRSmedir.Quaternion;
end
...
%Elimino los últimos segundos de la posición
%nrf=fix((0.1*t)/dt);

%posHP=posHP(1:(n-nrf),:);
save('medicion','tiempo','ax','ay','az','gx','gy','gz','ac_modFilt',...
'ac_umbral','vel','pos','posHP','quat','acel','dt','umbral','t','n')
guidata(hObject, handles);
